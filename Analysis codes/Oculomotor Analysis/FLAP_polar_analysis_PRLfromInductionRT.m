...
stopsAcc=1:5:(subjs*5);
  for ui=1:subjs
subj{ui}=overall_score_uab(stops(ui):stops(ui)+9,:);

  end

FLAP_pilot_ACC=overall_score_uab(1:2:end,:);
FLAP_pilot_RT=overall_score_uab(2:2:end,:);
...
score_PRLdist=NormframeDistributionPRL(ui,:);
