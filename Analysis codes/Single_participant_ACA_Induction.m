...
figure
 ...
 % average angle (across participants) for this specific task
score_induction = PRL_ind(ui,:); %mnread_scores(:,ui)';%PRL_test_scores_post_mean_av;
...
%score_VA = VAAcc_mean; %search_scores(:,ui)'; %cell2mat(average_post(:,ui))';
 score_VA = VAAcc(ui,:);
% angle of the participant for this specific task
score_crowdingR=CWRAcc(ui,:);
score_crowdingT=CWTAcc(ui,:);
scoreAttS=AttSAcc(ui,:);
scoreAttL=AttLAcc(ui,:);
...
lgt=legend(Pax, [SubNum{ui} ' PRL'],['VA'], ['CWrad'],['CWtan'], ['ExoAtt'], ['SustAtt'],'location', 'Northeast')
lgt.FontSize=13;
%title(Pax,'Average PRL location per task', 'fontsize',16)
