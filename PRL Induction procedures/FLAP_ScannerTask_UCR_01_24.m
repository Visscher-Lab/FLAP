...
% orientation discrimination and contour integration task (2 blocks of
...
try
    ...
    name= 'Parameters';
    numlines=1;
    defaultanswer={'test','1', '5', '1' , '2', '0', '0', '0', '1', '3'};
    ...
    % 1: gabor of shapes; 2: target location (left vs right), 3: shape type (6/9 or eggs)
    ...
    if demo==0 %PD added this part 8/15/23
        ...
        TRwait=ones(25,1);
        scanDur=133.5; %PD: 89TR 11/2/23
    else
        scanDur=282;%188 TR 11/2/23
    end
    %% HERE starts trial loop
    Bpress1=0; % response code for response box
    Bpress10=0; % response code for scanner trigger
    count1 = 0;
    count2 = 0;
    gaborcount = 0;
    for trial=1:length(mixtr)
        %% jitter code @ trial loop start
        if site == 5
            Datapixx('RegWrRd');
            ...
            TRpass=0; %JitterStartTimeD(trial);

            %    TRcount=TRcount+TRwait(trial);
            if trial > 1
                TRcount=TRwait(trial-1)*TR;
            end
            if trial==1
                fix = 0;
                while fix < TR
                    ...
                    Datapixx('RegWrRd');
                    fixD = Datapixx('GetTime');
                    fix = fixD - JitterStartTimeD(trial);
                end
                Datapixx('RegWrRd');
                JitterEndTimeD(trial)=Datapixx('GetTime');
                TRcounter(trial)= JitterEndTimeD(trial)- JitterStartTimeD(trial);
                TrialStartTimeD(trial)=JitterEndTimeD(trial);
            end

            if trial > 1 && mixtr(trial,1)~=9 % if it's not rest and trial is greater than 1
                if TRwait(trial-1)~=0
                    if TRwait(trial-1)<2 % if we have less than 2 TRs to wait, we DO NOT sync with TTL
                        while TRpass<TRcount %&& Bpress1==0 % wait until TRpass=TRcount, then
                            ...
                            %        TRpass=round((CurrentTimeD-JitterStartTimeD(trial))/TR);
                            TRpass=CurrentTimeD-JitterStartTimeD(trial);
                        end
                        count1 = count1+1;
                        countrial(count1)=trial;

                    elseif TRwait(trial-1)>1 %if we have more than 1 TR to wait, we DO sync with TTL
                        Bpress10=0;
                        ...
                        Datapixx('RegWrRd');
                        count2 = count2 + 1;
                        countrial2(count2)=trial;
                        CurrentTimeD(count2)=0;
                        Datapixx('RegWrRd');
                        pretrwait2whileloop(count2)=Datapixx('GetTime');
                        [Bpress10 timestamp2TR(count2)]= WaitForEvent_Jerry((TR*2), TheTrigger); % sync with next TR pulse
                        if Bpress10 > 0
                            Datapixx('RegWrRd');
                            prewaitCurrentTimeD(count2)=Datapixx('GetTime');
                        end
                        Datapixx('RegWrRd');
                        posttrwait2whileloop(count2)=Datapixx('GetTime');
                        while CurrentTimeD(count2)<prewaitCurrentTimeD(count2)+TR
                            Datapixx('RegWrRd');
                            CurrentTimeD(count2)=Datapixx('GetTime');
                        end
                    end
                end
            ...
            elseif trial > 1 && mixtr(trial,1)==9 % if it's REST, wait for 9 TTLs
                if TRwait(trial-1)~=0
                    if TRwait(trial-1)<2 % if we have less than 2 TRs to wait, we DO NOT sync with TTL
                        while TRpass<TRcount %&& Bpress1==0 % wait until TRpass=TRcount, then
                            %  [Bpress1 timestamp2]= WaitForEvent_Jerry(0, TheTrigger); % sync with next TR pulse
                ...
                Datapixx('RegWrRd');
                Datapixx('SetAudioSchedule',0,[Fs*2],length(restsound'),0,0,length(restsound'));
                Datapixx('StartAudioSchedule');
                Datapixx('RegWrRd'); %
                WaitSecs(TR)

                Datapixx('RegWrRd');
                starttesttime=Datapixx('GetTime');
                theTRs=0;
                thewait=9;
                Datapixx('EnableDinDebounce');                          % Debounce button presses
                Datapixx('SetDinLog');                                  % Log button presses to default address
                ...
                Datapixx('SetDinLog');
                Datapixx('RegWrRd');
                while theTRs<thewait
                    [Bpress10 timestamprest]= WaitForEvent_Jerry((TR), TheTrigger); % sync with next TR pulse
                    if Bpress10>0
                        theTRs=theTRs+1;
                        ttltimes(theTRs)=timestamprest;
                        diff(theTRs)=timestamprest-starttesttime;
                        Bpress10=0;
                    end
                end
                Datapixx('RegWrRd');
                restcount=Datapixx('GetTime');
                restpass= restcount;
                while restpass<restcount+TR %&& Bpress1==0 % wait until TRpass=TRcount, then
                    %  [Bpress1 timestamp2]= WaitForEvent_Jerry(0, TheTrigger); % sync with next TR pulse
                    Datapixx('RegWrRd');
                    restpass=Datapixx('GetTime');
                end
            end

        elseif site==6 || site == 7%PD added this part to count TRs 8/18/23
            kt(trial,1)=1;
            ...
            elseif mixtr(trial-1,1)==9 %PD: first trial after the rest block 8/18/23
                % PreviousCueTime=CueOnsetTime(trial-2,1);
                %TimeToStopListening(trial,1)=PreviousCueTime+(2*TR)+TRwait(trial-2,1)+TR+RestDuration; %PD:We will count TTLs during rest(TTL_timeforrest)not TRwaits (TTL_time) 10/31/23
                %TrialStartTime(trial,1)=PreviousCueTime+(2*TR)+TRwait(trial-2,1)+TR+RestDuration;
                TimeToStopListening(trial,1)=TrialStartTime(trial-1,1)+RestDuration; %PD:We will count TTLs during rest(TTL_timeforrest)not TRwaits (TTL_time) 10/31/23
        ...
        if site ~= 5 
            if trial==1 %editing TrialStartTime after TRwait. It's necessary because sometimes we wait for audio info for additional 1TR long
                TrialStartTime(1,1)=startTime+(2*TR);%one TR wait at the beginning of the experiment, one TR wait for the audio info
            elseif mixtr(trial,1)~=mixtr(trial-1,1) || (trial==50 && gaborordercol(2)==5 && whichRun==4) || (trial==50 && gaborordercol(2)==5 && whichRun==5) || (trial==17 && eggordercol==2 && whichRun==5) %PD:if there is an audio info before this trial 10/31/23
                TrialStartTime(trial,1)=TrialStartTime(trial,1)+TR; %if there is audio info before this trial
            end
        end
        ...
        whichblock=GetSecs;
        if (trial == 1 && mixtr(trial,1) == 1) || (trial > 1 && mixtr(trial,1) ~= mixtr(trial-1,1) && mixtr(trial,1) == 1) || (trial > 1 && mixtr(trial,1) == 1 && gaborcount == 17)
            if site == 6 || site ==7
                PsychPortAudio('FillBuffer',pahandle,gaborsound')
            else
                Datapixx('WriteAudioBuffer', gaborsound', 0); % loads data into buffer
                Datapixx('RegWrRd'); %
                Datapixx('SetAudioSchedule',0,[Fs*2],length(gaborsound'),0,0,length(gaborsound'));
                Datapixx('RegWrRd'); %
                Datapixx('StartAudioSchedule');
                Datapixx('RegWrRd');
            end
        ...
        elseif (trial == 1 && mixtr(trial,1) == 2 && mixtr(trial,3) == 1) || (trial > 1 && (mixtr(trial,1) ~= mixtr(trial-1,1) || mixtr(trial,3) ~= mixtr(trial-1,3)) && mixtr(trial,1) == 2 && mixtr(trial,3) == 1)
            if site == 6 || site == 7
                PsychPortAudio('FillBuffer',pahandle,eggsound')
            else
                Datapixx('WriteAudioBuffer', eggsound', 0); % loads data into buffer
                Datapixx('RegWrRd'); %
                Datapixx('SetAudioSchedule',0,[Fs*2],length(eggsound'),0,0,length(eggsound'));
                Datapixx('RegWrRd'); %
                Datapixx('StartAudioSchedule');
                Datapixx('RegWrRd');
            end
        ...
        elseif trial > 1 && mixtr(trial,1) == 9
            if site == 6 || site == 7
        ...
        elseif (trial == 1 && mixtr(trial,1) == 2 && mixtr(trial,3) == 2) || (trial > 1 && (mixtr(trial,1) ~= mixtr(trial-1,1) || mixtr(trial,3) ~= mixtr(trial-1,3)) && mixtr(trial,1) == 2 && mixtr(trial,3) == 2)
            if site == 6 || site == 7
                PsychPortAudio('FillBuffer',pahandle,dsound')
            else
                Datapixx('WriteAudioBuffer', dsound', 0); % loads data into buffer
                Datapixx('RegWrRd'); %
                Datapixx('SetAudioSchedule',0,[Fs*2],length(dsound'),0,0,length(dsound'));
                Datapixx('RegWrRd'); %
                Datapixx('StartAudioSchedule');
                Datapixx('RegWrRd'); %
            end
        ...
        if trial==length(mixtr)
            if site == 5
                Datapixx('RegWrRd');
                ExpEndTimeD=Datapixx('GetMarker');
            else
            ...
            if (eyetime2-trial_time) > preCueISI && (eyetime2-trial_time)<preCueISI+CueDuration && stopchecking>1
                ...
                if site == 5 % use DATApixx to play audio @ CAN, AYS 4/29/23
                    if mixtr(trial,2)==1
                        ...
                        Datapixx('SetAudioSchedule',0,[Fs*2],length(bip_sound_left'),1,0,length(bip_sound_left'));
                    elseif mixtr(trial,2)==2
                        ...
                        Datapixx('SetAudioSchedule',0,[Fs*2],length(bip_sound_right'),2,0,length(bip_sound_right'));
                    end
                    %                                         if playsound==1 && (eyetime2-trial_time)>0.1
...
%                     imagearray{trial}=Screen('GetImage', w);
            ...
            elseif (eyetime2-trial_time)>=preCueISI+CueDuration+postCueISI+StimulusDuration+PostStimulusDuration
                ...
                if site == 5
                    Datapixx('RegWrRd');
                    trialEndTime(trial)=Datapixx('GetTime');%
                end
            end
    ...
    if trial==length(mixtr) %PD added this last wait after last trial ends 9/7/23
        lasttrialwait=GetSecs;% if GetSecs-startTime < 282; %total duration should be 282
        %while (trialstart_frame(65)-startTime)+(2*TR)+(TRwait(trial)*TR) < 282
        % while (lasttrialwait-startTime) + (TRwait(trial)*TR) < scanDur %waits for the last TRwait long
        %while GetSecs<trialstartframe(trial)+(2*TR)+(TRwait(trial)*TR)
        if site == 5
            Datapixx('RegWrRd')
            EndTask = Datapixx('GetTime');
            lasttrialwait=EndTask;
            while EndTask-startTime<scanDur
                Datapixx('RegWrRd')
                EndTask = Datapixx('GetTime');
                fixationscriptW
            end
            ...
            while GetSecs-startTime<scanDur
            end
            DrawFormattedText(w, 'Task completed - Press a key to close', 'center', 'center', white);
        end
        %end
    end
    lasttrialwaitends=GetSecs;
    %     DrawFormattedText(w, 'Task completed - Press a key to close', 'center', 'center', white);
    ListenChar(0);
