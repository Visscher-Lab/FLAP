function [bigImage, cmap] = createMontage(imgSrc, thumbnailSize, montageSize, borderSize, backgroundColor, indices, cmap, waitbarEnabled)
...
function I = getImages(imgSource, thumbnailSize, borderSize, backgroundColor, idxs, cmap, waitbarEnabled)
...
if isempty(thumbnailSize)
   thumbnailSize = [size(I,1), size(I,2)];
    ...
    %Replace all Infs with Nans as imresize only supports NaN
    thumbnailSize(~isfinite(thumbnailSize)) = NaN;
    if any(~isfinite(thumbnailSize))
