...
try
    prompt={'Participant name', 'day','site? UCR(1), UAB(2), Vpixx(3)','scotoma old mode active','scotoma Vpixx active', 'demo (0) or session (1)', 'Locations: (2) or (4)',  'eye? left(1) or right(2)','Calibration? yes (1), no(0)', 'Task: acuity (1), crowding (2), exo attention (3)'};
...
if whichTask==1
    ...
    totalmixtr=length(mixtrVA)+length(mixtrCW);
       ...
       elseif whichTask==2
            totalmixtr=length(mixtrCW);
       elseif whichTask==3
          totalmixtr=length(mixtrAtt);
       end
    %% main loop
    HideCursor;
    ListenChar(2);
    ...
    for trial=1:totalmixtr
      ...
      if trial==1
        ...
        if mod(trial,tr_per_condition+1)==0 && trial~= length(totalmixtr)+1 
        ...
        if trialTimedout(trial)== 0
            ...
            elseif whichTask == 2
                ...
                if foo(theans(trial))
                    ...
                    if reversalsCW(mixtrCW(trial,1),mixtrCW(trial,2)) <2
                        sc.down= sc.steps(1);
                    elseif reversalsCW(mixtrCW(trial,1),mixtrCW(trial,2)) >= 2
                        sc.down= sc.steps(2);
                    end
                    if corrcounterCW(mixtrCW(trial,1),mixtrCW(trial,2))==sc.down && staircounterCW(mixtrCW(trial,1),mixtrCW(trial,2))>sc.down % we don't want to call the first scale down a reversal
