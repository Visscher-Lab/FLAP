...
dinRed      = hex2dec('1'); % 1 left
dinYellow   = hex2dec('2'); % 2 right
dinGreen    = hex2dec('4'); % 4 up 
dinBlue     = hex2dec('8'); % 8 down
...
RespType=[1 2 3 4 11];
    ...
    if EyetrackerType==1
        eyelinkCalib
    elseif EyetrackerType==2 %added by AS 4-28-23, you can put into a script if you wish
       if Calibrate
  ...
  if AssessmentType==1
        ...
        while eyechecked<1
              ...
              %% here is where the first time-based trial loop starts (until first forced fixation is satisfied)
                ...
                elseif (eyetime2-trial_time)>=preCueISI && (eyetime2-trial_time)<preCueISI+CueDuration && stopchecking>1
                   ...
                   if exist('startrial') == 0
                    ...
                    if useDatapixx % use DATApixx to play audio @ CAN, AYS 4/29/23
                        if mixtr(trial,2)==1
          ...
          if restscreen==0
            ...
            if useDatapixx % DATApixx AYS 5/4/23 I added some documentation for WaitForEvent_Jerry - let me know if you have questions.
                [Bpress, RespTime, thekeys] = WaitForEvent_Jerry(responseduration, TargList, stim_startD(trial));
                
%                                    [thekeys secs] = Datapixx('ReadDinLog');

              %  [Bpress RespTime keyCode] = getEventPixx(TargList,starttime)
    ...
    if useDatapixx && EyetrackerType==2 && Datapixx('IsReady') % AYS 5/4/23
        Datapixx('StopTPxSchedule');
        status = Datapixx('GetTPxStatus');
        toRead = status.newBufferFrames;
