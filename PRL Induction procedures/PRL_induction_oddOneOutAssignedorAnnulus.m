...
try
    ...
    BITS=2; % (1=Bits++; 2=Display++; 0=no bits)
    ...
    baseName=['./data/' SUBJECT '_DAY_' num2str(expday) '_PRL_induction_oddoneout_' TYPE '_' num2str(scotomadeg) ' deg ' num2str(c(1)-2000) '_' num2str(c(2)) '_' num2str(c(3)) '_' num2str(c(4)) '_' num2str(c(5))]; %makes unique filename
    ...
    elseif BITS==2; %UAB
        ...
        %         fclose(s1)
        %         clear s1
        % commented above out July 1, 2021
        screencm=[69.8, 35.5];
        v_d=57;
        AssertOpenGL;
        ...
        PsychImaging('AddTask', 'General', 'FloatingPoint32Bit');
        % PsychImaging('AddTask', 'General', 'EnableBits++Mono++Output');
        % % commented out above line on July 1, 2021
        %     PsychImaging('AddTask', 'FinalFormatting','DisplayColorCorrection','LookupTable');
    ...
    DrawFormattedText(w, 'report which stimulus is different from the others by pressing \n \n f(north-west) \n \n j (north-east) \n \n c (south-west) \n \n m (south-east)  \n \n \n \n Press any key to start', 'center', 'center', white);
    Screen('Flip', w);
    KbWait;
    %  Screen('DrawLine', w, colorfixation, wRect(3)/2, wRect(4)/2-fixationlength, wRect(3)/2, wRect(4)/2+fixationlength, 4);
    %  Screen('DrawLine', w, colorfixation, wRect(3)/2-fixationlength, wRect(4)/2, wRect(3)/2+fixationlength, wRect(4)/2, 4);
    ...
    for trial=1:length(mixtr)
        ...
        if totalelements == 3
            theans(trial)=randi(3); %generates answer for this trial
        ...
        if randomposition == 1
            ...
            target_ecc_x=positionmatrix(posmatrix(theans(trial)),1);
            target_ecc_y=positionmatrix(posmatrix(theans(trial)),2);
            
            newnewpos=posmatrix;
            newnewpos(theans(trial),:)=[];
        ...
        % if theSmile(trial) == 1
        % disTexture(trial)= theFace(2);
        % elseif theSmile(trial) == 2
        % disTexture(trial)= theFace(1);
        % end
        ...
        while eyechecked<1
            ...
            elseif (eyetime2-trial_time)>waittime+ifi*4 && (eyetime2-trial_time)<waittime+ifi*6 && fixating>400 && stopchecking>1 && keyCode(RespType(1)) + keyCode(RespType(2)) + keyCode(RespType(3)) + keyCode(RespType(4)) + keyCode(escapeKey)== 0 %present pre-stimulus and stimulus
            ...
            elseif (eyetime2-trial_time)>=waittime+ifi*6 && fixating>400 && stopchecking>1 && keyCode(RespType(1)) + keyCode(RespType(2)) +keyCode(RespType(3)) + keyCode(RespType(4)) + keyCode(escapeKey)== 0 %present pre-stimulus and stimulus
                %       toc
                %     disp('stimulus')
            ...
            elseif (eyetime2-trial_time)>=waittime+ifi*7 && fixating>400 && stopchecking>1 && keyCode(RespType(1)) + keyCode(RespType(2)) + keyCode(RespType(3)) + keyCode(RespType(4)) + keyCode(escapeKey)~= 0 && mostratarget>10 % wait for response
                %    tic
                %     toc
                %   disp('afterkeypress')
                stim_stop=GetSecs;
                            ...
                            if  exist('EyeCode','var')
                                if length(EyeCode)>6 %&& circlefix>6
                                    ...
                                    if sum(EyeCode(end-6:end))~=0
                                        ...
                                        %  if circlefix2(gh)<5
                                        % Screen('DrawTexture', w, Neutralface, [], imageRect_offs);
                                        % commented out above July 1, 2021
                                        imageRect_offscue=[imageRectcue(1)+(newsamplex-wRect(3)/2)+PRLxpix, imageRectcue(2)+(newsampley-wRect(4)/2)+PRLypix,...
                                            imageRectcue(3)+(newsamplex-wRect(3)/2)+PRLxpix, imageRectcue(4)+(newsampley-wRect(4)/2)+PRLypix];
                                        if visibleCircle ==1
                                            Screen('FrameOval', w,200, imageRect_offscue, oval_thick, oval_thick);
                                ...
                                if  exist('EyeCode','var')
                                    ...
                                    if length(EyeCode)>5 && circlefix2(gg)>5
                                        ...
                                        if sum(EyeCode(end-5:end))~=0
                                            ...
                                            % commenting out above in the hopes that this will
                                            % make the image stable on screen kmv July 1,
                                            % 2021
                                            imageRect_offscue=[imageRectcue(1)+(newsamplex-wRect(3)/2)+PRLxpix, imageRectcue(2)+(newsampley-wRect(4)/2)+PRLypix,...
                                                imageRectcue(3)+(newsamplex-wRect(3)/2)+PRLxpix, imageRectcue(4)+(newsampley-wRect(4)/2)+PRLypix];
                                            if visibleCircle ==1
