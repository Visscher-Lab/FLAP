...
try
    prompt={'Participant Name', 'Pre (1) vs Post (2)','site? Vpixx(3), Scanner (5)', 'Practice (0) or Session(1)', 'Eye? left(1) or right(2)', 'Calibration? yes (1), no(0)', 'Scotoma? yes (1), no(0)', 'Eyetracker(1) or mouse(0)?', 'response box (1) or keyboard (0)', 'which run?(1-6)' };
    
    name= 'Parameters';
    numlines=1;
    defaultanswer={'test','1', '5', '1' , '2', '0', '1', '0', '1', '1'};
    answer=inputdlg(prompt,name,numlines,defaultanswer);
    if isempty(answer)
        return;
    end
    whichRun=str2num(answer{10,:});
    ...
    condtr=activeblocktype(whichRun,:);
    possiblecond=[1 1;
        2 1;
        2 2;
        9 9];
