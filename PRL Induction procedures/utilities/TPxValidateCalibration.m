function calib_appected = TPxValidateCalibration(targets, isTPX, screenHandle, initRequired)
    if nargin < 4
        initRequired = 1;
    end
    ...
    if(~skip_validation)
        ...
        while (1)
            ...
            if (~file_recorded)
                ...
                imwrite(imageArray, 'average_error.jpg');
            end
            ...
            if(~file_recorded)
                imageArray = Screen('GetImage', windowPtr);
                imwrite(imageArray, 'reported_gaze_left.jpg')
            end
            [secs, keyCode, deltaSecs] = KbWait;
            if keyCode(KbName('Y'))
                calib_appected = 1;
            ...
            if(~file_recorded)
                imageArray = Screen('GetImage', windowPtr);
                imwrite(imageArray, 'reported_gaze_right.jpg')
                file_recorded = 1;
            end
            [secs, keyCode, deltaSecs] = KbWait;
            if keyCode(KbName('Y'))
