...
if (((time_now - start_time) > 1))% && ~recording)
    ...
    if (recording == 1)
        ...
        if ((t2 - t) > 1/120) % points presented every 8.333 msec
            [bufferData(u, :)] = Datapixx('GetTPxMiniLastEyePosition');
            u = u + 1;
            curr_time = GetSecs;
            if ((curr_time - start_rec) > 10)
                finish_time = GetSecs;
                break;
                save data
            end
...
bufferData2 = Datapixx('GetTPxMiniRecordedData');

% start_rec = GetSecs;
% curr_time = GetSecs;
% start = Datapixx('ReadTPxMiniData');
% start_rec = start(1,1);
% %u = 1;
% while (curr_time - start_rec) < 15
%         tic
%         [bufferData(u, :)] = Datapixx('ReadTPxMiniData');
%         %[xScreenRight yScreenRight xScreenLeft yScreenLeft xRawRight yRawRight xRawLeft yRawLeft bufferSecs(u)] = Datapixx('GetEyePosition');
%         u = u + 1;
%         %[bufferData(u, :)] = Datapixx('ReadTPxMiniData');
%         bufferSecs(u - 1) = toc;
%         curr_time = bufferData(u-1, 1);
%         
% end
% 
% finish_time = GetSecs;
