...
%     counter=counter+0;
% end
if framecounter>1 && round(newsampley - wRect(4)/2) > startingfixationpoint(mixtr(trial,1)) * pix_deg -fixwindowPix...
   && round(newsampley - wRect(4)/2) < startingfixationpoint(mixtr(trial,1)) * pix_deg +fixwindowPix...
    && round(newsamplex-(wRect(3)/2)) > -fixwindowPix ...
    && round(newsamplex-(wRect(3)/2))<= fixwindowPix   
    % if eyes are within fixation window; we count the frame
    fixating=fixating+1;
    counter=counter+1;
