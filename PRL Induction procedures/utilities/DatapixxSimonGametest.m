%function [thedata]=DatapixxSimonGametest()
% DatapixxSimonGame()
%
...
doutYellow     = hex2dec('00020000');

doutBlue=hex2dec('00090000');
...
for iRep = 1:nReps
    ...
    if (bitand(condList(iRep), 1))
        Datapixx('SetDinDataOut', doutBlue);
        correctResponse = dinBlue;
    else
    ...
    if (bitand(condList(iRep), 2))
        ...
        [datta timetags] = Datapixx('ReadDinLog');
        if (datta(1) ~= correctResponse)
