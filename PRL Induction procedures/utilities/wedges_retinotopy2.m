function stimulus = wedges_retinotopy2(saveInfo,sparams,TR,scanDur,display,tChar,rChar)
%% Retinotopy Wedge stimuli
%
%   Usage:
%   wedges_retinotopy(saveInfo,stimParams,TR,scanDur,display,tChar,rChar)
%
%   Required inputs:
...
if responsebox==0
    ...
    params.FlickerTimes         = [256,400,550,710,1200,1320,1523,1810,2109,2367];
    disp(['Trigger received - ' params.startDateTime]);
    % flush response box events
    ...
    while GetSecs-startTime < scanDur && ~breakIt  %loop until 'esc' pressed or time runs out
      ...
      [Bpress, RespTime, TheButtons, ~, ~,PTBRespTime] = DontWaitForEvent_Jerry4(TargList, Bpress, TheButtons, RespTime,timestamp, binaryvals,inter_buttonpress, bin_buttonpress, inter_timestamp,PTBRespTime);
        ...
        if abs((elapsedTime / (TR / 10 )) - curFrame) > 0
            curFrame = ceil( elapsedTime / (TR / 10 ));
        end
