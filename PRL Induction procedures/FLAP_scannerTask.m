        ...
        if blocktype==4 % rest
            ...
            if restorder==1
                ...
                while GetSecs < RestTime + (rest_duration-0.500); %  rest for 15 sec
                    ...
                    if  keyIsDown
                        ...
                        keypresstimer(1,kr)=keyTime;
                        if keyr==KbName('t');
                            if kr==1 && keypresstimer(1,kr)<startTime+pastexptime
                            elseif kr==1 && keypresstimer(1,kr)>startTime+pastexptime
                                TTL_timeforrest(jr)=keyTime;
                                kr=kr+1;
                                jr=jr+1;
                            elseif keypresstimer(1,kr)-keypresstimer(1,kr-1)>=.5 %I added this because kbcheck gets ~30 keypresses since manual testing isn't quick enough
            ...
            for trial=1:totaltrial
                ...
                if trial==1 && totalblock==1 % it's the first trial of the first block
                    itiprevious = 1;
                    TimeToStopListening(totalblock,trial)=startTime+((itiprevious-0.5)*TR);
                ...
                elseif trial==1 && totalblock>1 && activeblocktype(runnumber,totalblock-1)==4% it's the first trial of the following blocks and previous block is rest
                    TimeToStopListening(totalblock,trial)=RestTime+rest_duration;
               ...
               if cond==1
                clear keyIsDown keyTime keyCode
                while GetSecs < TimeToStopListening(totalblock,trial)%listens TTL pulses
                ...
                if blocktype==1 %gabors
                    ...
                    if stimulusdirection_leftstim==1 %left oriented
                    ...
                    if stimulusdirection_rightstim==1 %left orited
                    ...
                    responseScanner2;
                elseif blocktype==2 %egg-CIclear
                    CIstimuliMod_Scanner
                    ...
                    Screen('FrameOval', w,gray, imageRect_offsCImaskleft, maskthickness*(3/4), maskthickness*(3/4)); %Screen('FrameOval', w,gray, imageRect_offsCImaskleft, maskthickness/2, maskthickness/2);
                    Screen('FrameOval', w,gray, imageRect_offsCImaskright, maskthickness*(3/4), maskthickness*(3/4));
                ...
                elseif blocktype==3 %6 or 9
                    ...
                    Screen('FrameOval', w,gray, imageRect_offsCImaskleft, maskthickness*(3/4), maskthickness*(3/4));
                    Screen('FrameOval', w,gray, imageRect_offsCImaskright, maskthickness*(3/4), maskthickness*(3/4));
