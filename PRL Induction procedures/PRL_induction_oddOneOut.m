% PRL induction procedure with simulated scotoma, multiple targets
...
try
    ...
    expdayeye = answer{2,:};
    site=answer{3,:};
    BITS=str2num(site); % (1=Bits++; 2=Display++; 0=no bits)
%     if BITS==1
...
%     end
    ...
    max_distractors=15;
     ...
     stimulussize=3; %size of the stimulus
    ...
    distancedeg=2.5; % distance among stimuli
...
trialTimeout=timeOut+3;
      ...
      scotomadeg=10;
            baseName=['./data/' SUBJECT '_DAY_' num2str(expday) '_PRL_induction_OddOneOut' TYPE '_' num2str(scotomadeg) ' deg ' num2str(c(1)-2000) '_' num2str(c(2)) '_' num2str(c(3)) '_' num2str(c(4)) '_' num2str(c(5))]; %makes unique filename
...
%     
 ...
 % [x,y]=meshgrid(-imsize:imsize,-imsize:imsize);
  ...
  %  circle = x.^2 + y.^2 <= xylim^2;
   % [nrw, ncl]=size(x);
   %         img=imresize(img,[nrw nrw],'bicubic');

  %img = double(circle) .* double(img)+bg_index * ~double(circle);
...
%     [elementcoordx,elementcoordy] = pol2cart(theta,rho);
    ...
    angles = [15 75 1-5 165 195 255 285 345];
    ...
    for i=1:trials
     ...
     %   newangles(find(thesetDist(whichLoc(i)))) = [];
        ...
        thesetTgt=thesetDist;
              %  thesetTgt2=thesetTgt;
oui(i)=newangles(randi(length(newangles)));
        thesetTgt(whichLoc(i))=newangles(randi(length(newangles)));
     %   set_tgt2{i}=thesetTgt2;
                set_tgt{i}=thesetTgt;
      ...
      %  theTargets{i}=largerout2;
      %  theOthers{i}=largerout;
    ...
    for ui=1:trials
        ...
        matrixtarget=[ matrixtarget; set_tgt{ui}];
    
    end

        matrix=[pos_one' pos_two' pos_three' pos_four'];
...
for aux=1:length(PRLx)
        ...
        while eyechecked<1
            ...
            elseif (eyetime2-trial_time)>=waittime+ifi*6 && fixating>400 && (eyetime2-pretrial_time)<=trialTimeout && stopchecking>1 && keyCode(RespType(1)) + keyCode(RespType(2)) + keyCode(escapeKey)== 0 %present pre-stimulus and stimulus
            ...
            elseif (eyetime2-trial_time)>=waittime+ifi*7 && fixating>400 && (eyetime2-pretrial_time)<=trialTimeout && stopchecking>1 && keyCode(RespType(1)) + keyCode(RespType(2))  + keyCode(escapeKey)~= 0 && mostratarget>10 % wait for response
                ...
                cicci=100;
            elseif (eyetime2-trial_time)>=waittime+ifi*8 && fixating>400 && (eyetime2-pretrial_time)<=trialTimeout && stopchecking>1 && keyCode(escapeKey)~= 0 % wait for response
                stim_stop=GetSecs;
                thekeys = find(keyCode);
                thetimes=keyCode(thekeys);
                [secs  indfirst]=min(thetimes);
                                    ...
                                    if length(EyeCode)>6 
                                        ...
                                        elseif sum(EyeCode(end-5:end))==0
                                          ...
                                          %              countertargettteyeX(suis,nn)=1;
                                                    ...
                                                    countertargettt(suis,nn)=99;
                                                end
                                            end
                             %           countertargettt(aux,nn)=1;
                                           %line below is TO TEST UNCOVERING OF TARGET 
                                        %    Screen('DrawTexture', w, Distractorface, [], imageRect_offs);
                      %          Screen('DrawTexture', w, Neutralface, [], imageRect_offs);
                    ...
                    for gg=1:totalelements-1
                        ...
                        for aux=1:length(PRLxpix)
                            ...
                            if stimpresent>0 && round(wRect(4)/2+(newsampley-(wRect(4)/2+neweccentricity_Yd(gg)))+PRLypix(aux))<wRect(4) && round(wRect(4)/2+(newsampley-(wRect(4)/2+neweccentricity_Yd(gg)))+PRLypix(aux))>0 && round(wRect(3)/2+(newsamplex-(wRect(3)/2+neweccentricity_Xd(gg))+PRLxpix(aux)))<wRect(3) && round(wRect(3)/2+(newsamplex-(wRect(3)/2+neweccentricity_Xd(gg))+PRLxpix(aux)))>0 %|| ...
                                    ...
                                    if  exist('EyeCode','var')
                                        %     circlefix2(gg)=0;
                                        if length(EyeCode)>5 && circlefix2(aux,gg)>5
                                            circlefix2(aux,gg)=circlefix2(aux,gg)+1;
                                          ...
                                          %      countertargetdist(gg,aux,nn)=1;
                                                ...
                                                for gei = 1:totalelements-1
                                                    for suis=1:length(PRLxpix)
                                                        if round(wRect(4)/2+(newsampley-(wRect(4)/2+neweccentricity_Yd(gei)))+PRLypix(suis))<=size(circlePixels,1) && round(wRect(3)/2+(newsamplex-(wRect(3)/2+neweccentricity_Xd(gei)))+PRLxpix(suis))<=size(circlePixels,2) ...
                                                                &&  round(wRect(4)/2+(newsampley-(wRect(4)/2+neweccentricity_Yd(gei)))+PRLypix(suis))> 0 && round(wRect(3)/2+(newsamplex-(wRect(3)/2+neweccentricity_Xd(gei)))+PRLxpix(suis))> 0;
                                                            %  if circlePixels(codey(suis), codex(suis))>0.781
                                        ...
                                        elseif length(EyeCode)<5 && circlefix2(aux,gg)<=5
                                            Screen('DrawTexture', w, Neutralface, [], imageRect_offsDist{gg});
...
%             end;
EyeSummary.(TrialNum).Elements=imageRect_offsDist;
