...
try
    ...
    for trial=1:totalmixtr
        ...
        % randomize O location depending on the subsequent C's location
        removeloc=mixtr(trial,1);
        newlocs=thelocs(thelocs~=removeloc);
        pickloc=newlocs(randi(length(newlocs)));
        theeccentricity_XO=eccentricity_X(pickloc);
        theeccentricity_YO=eccentricity_Y(pickloc);
        
        imageRect_offso=[imageRect(1)+theeccentricity_XO, imageRect(2)+theeccentricity_YO,...
            imageRect(3)+theeccentricity_XO, imageRect(4)+theeccentricity_YO];
        ...
        while eyechecked<1
            ...
            elseif (eyetime2-trial_time)>=trialonsettime+ifi*2+cueonset+circleduration && (eyetime2-trial_time)<trialonsettime+ifi*2+cueonset+currentcueduration+circleduration && fixating>400 && stopchecking>1 && (eyetime2-pretrial_time)<=trialTimeout
                % compute and present cue
                
                if mixtr(trial,2)== 1 %exo cue
                    if mixtr(trial,1)==  1
                        subimageRect_offs_cue=imageRect_offs_cue(1:4,:);
                    elseif mixtr(trial,1)==  2
                        subimageRect_offs_cue=imageRect_offs_cue(5:8,:);
                    elseif mixtr(trial,1)==  3
                        subimageRect_offs_cue=imageRect_offs_cue(9:12,:);
                    else
                        subimageRect_offs_cue=imageRect_offs_cue(13:16,:);
                    end
                    
                end
                if mixtr(trial,2)== 3 %endo cue
                    
                    whichDot=((pickloc*4)-3); % which diamond?
                    
                    if pickloc==1 & mixtr(trial,1)== 2 || pickloc==3 & mixtr(trial,1)== 2 || pickloc==4 & mixtr(trial,1)== 2
                        subimageRect_offs_cue=imageRect_offs_cue(whichDot+1,:);
                    elseif pickloc==1 & mixtr(trial,1)== 3 || pickloc==2 & mixtr(trial,1)== 3 || pickloc==4 & mixtr(trial,1)== 3
                        subimageRect_offs_cue=imageRect_offs_cue(whichDot+2,:);
                    elseif pickloc==1 & mixtr(trial,1)== 4 || pickloc==2 & mixtr(trial,1)== 4 || pickloc==3 & mixtr(trial,1)== 4
                        subimageRect_offs_cue=imageRect_offs_cue(whichDot+3,:);
                    elseif pickloc==2 & mixtr(trial,1)== 1 || pickloc==3 & mixtr(trial,1)== 1 || pickloc==4 & mixtr(trial,1)== 1
                        subimageRect_offs_cue=imageRect_offs_cue(whichDot,:);
                    end
