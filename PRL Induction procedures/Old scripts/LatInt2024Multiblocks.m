...
%RTBox('clear');a
try
    prompt={'Participant Name', 'day','site? UCR(1), UAB(2), Vpixx(3)', 'Task type (1: Lat int, 2: noise, 3: ori, 4: symmetrical dots)', 'Demo? (1:yes, 2:no)', 'Eye? left(1) or right(2)', 'Calibration? yes (1), no(0)', 'Scotoma? yes (1), no(0)', 'Eyetracker(1) or mouse(0)?', 'left (1) or right (2) TRL?' , 'blocks'};
    
    name= 'Parameters';
    numlines=1;
    defaultanswer={'test','1', '3', '3', '2' , '2', '1', '1', '1', '2', '8'};
...
%         end
        ...
        trial=kk;
                ...
                save([baseName 'block' num2str(ii)],'-regexp', '^(?!(wavedata|sig|tone|G|m|x|y|xxx|yyyy)$).');
                threshBlock{ii}=permute(Threshlist, [3 1 2]);
                rispoBlock{ii}=rispo;
                revcountBlock{ii}=revcount;
end
 ...
 for ui=1:blockNum   
     subplot(3,3,ui)
     scatter(1:length(threshBlock{ui}), threshBlock{ui})
%     hold on
