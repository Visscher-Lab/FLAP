...
try
    ...
    if whichTask~=3
        ...
        tr_per_condition=6;  %50
        thresh(1:PRLlocations, 1)=15; %25;
...
if whichTask==2
        %attention
        mixtrtemp=repmat(fullfact([2 2]),tr_per_conditionAtt,1);
...
%             theeccentricity_X=eccentricity_X(mixtr(trial));
%             theeccentricity_Y=eccentricity_Y(mixtr(trial)); 
            trialTimeout=realtrialTimeout;            

        end
        if whichTask ==1
            VAsize = Sizelist(thresh(mixtr(trial)));
...
%             theeccentricity_Y=eccentricity_Y(mixtr(trial,1));
        ...
        if trialTimedout(trial)== 0
            ...
            if whichTask == 1
          %      staircounter(mixtr(trial))=staircounter(mixtr(trial))+1;
            %    Threshlist(mixtr(trial),staircounter(mixtr(trial)))=VAsize;
                ...
                if foo(theans(trial))
                    ...
                    corrcounter(mixtr(trial))=corrcounter(mixtr(trial))+1;
                    if reversals(mixtr(trial))<2
                        sc.down=sc.steps(1);
                    elseif reversals(mixtr(trial))>= 2
                        sc.down=sc.steps(2);
                    end
                    if corrcounter(mixtr(trial))==sc.down && staircounter(mixtr(trial))>sc.down % we don't want to call the first scale down a reversal
                        isreversals(mixtr(trial))=1;
                        %  rever(trial)=1;
                        
                    end
                    if corrcounter(mixtr(trial))==sc.down %&& reversals(mixtr(trial))>= 2
                        % non streaking after 3 reversals
                        if isreversals(mixtr(trial))==1 %&& Threshlist(staircounter(mixtr(trial-sc.down)))<Threshlist(staircounter(mixtr(trial)))
                            if resetcounter(mixtr(trial))==0
                                reversals(mixtr(trial))=reversals(mixtr(trial))+1;
                                resetcounter(mixtr(trial))=1;
                                rever(trial)=1;
                            end
                            isreversals(mixtr(trial))=0;
                        end
                        thestep=min(reversals(mixtr(trial))+1,length(stepsizesVA));
                        if thestep>length(stepsizesVA)
                            thestep=length(stepsizesVA);
                        end
                        corrcounter(mixtr(trial))=0;
                        thresh(mixtr(trial))=thresh(mixtr(trial)) +stepsizesVA(thestep);
                        %      threshVA(mixtr(trial))=min(threshVA(mixtr(trial)),length(Sizelist));
                    end
                    thresh(mixtr(trial))=min(thresh(mixtr(trial)),length(Sizelist));
                elseif (thekeys==escapeKey) % esc pressed
                    ...
                    if  corrcounter(mixtr(trial))>=sc.down
                        isreversals(mixtr(trial))=1;
                    end
                    ...
                    thestep=min(reversals(mixtr(trial))+1,length(stepsizesVA));
                    thresh(mixtr(trial))=thresh(mixtr(trial)) -stepsizesVA(thestep);
                    thresh(mixtr(trial))=max(thresh(mixtr(trial)));
                    if thresh(mixtr(trial))<1
                        thresh(mixtr(trial))=1;
                    end
                end
            elseif whichTask == 2
                ...
                if foo(theans(trial))
                    resp = 1;
                    nswrCW(mixtr(trial,1),mixtr(trial,2),staircounter(mixtr(trial,1),mixtr(trial,2)))=1;
                    PsychPortAudio('FillBuffer', pahandle, corrS' ); % loads data into buffer
                    PsychPortAudio('Start', pahandle);
                    ...
                    if corrcounter(mixtr(trial,1),mixtr(trial,2))==sc.down && staircounter(mixtr(trial,1),mixtr(trial,2))>sc.down % we don't want to call the first scale down a reversal
                        isreversals(mixtr(trial,1),mixtr(trial,2));
                    end
                    ...
                    if corrcounter(mixtr(trial,1),mixtr(trial,2))==sc.down %&& reversals(mixtr(trial,1),mixtr(trial,2)) >= 2
                        ...
                        thestep=min(reversals(mixtr(trial,1),mixtr(trial,2))+1,length(stepsizesCW));
                        if thestep>length(stepsizesCW)
                            thestep=length(stepsizesCW);
                        end
                        corrcounter(mixtr(trial,1),mixtr(trial,2))=0;
                        thresh(mixtr(trial,1),mixtr(trial,2))=thresh(mixtr(trial,1),mixtr(trial,2)) +stepsizesCW(thestep);
                ...
                elseif (thekeys==escapeKey) % esc pressed
                    ...
                    resp = 0;
                    nswrCW(mixtr(trial,1),mixtr(trial,2),staircounter(mixtr(trial,1),mixtr(trial,2)))=0;
            ...
            if whichTask==1
                ...
                staircounter(mixtr(trial))=staircounter(mixtr(trial))+1;
                Threshlist(staircounter(mixtr(trial)))=VAsize;
                nswr(trial)=0;
                resetcounter(mixtr(trial))=0;
                if reversals(mixtr(trial))<2
                    sc.down=sc.steps(1);
        ...
        if whichTask==1
            ...
            contacorr(trial)=corrcounter(mixtr(trial));
        elseif whichTask ==2
            separation(kk)=sep;
