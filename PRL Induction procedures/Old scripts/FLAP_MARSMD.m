...
try
    ...
    CommonParametersMARS % load parameters for time and space
    scotomadeg=0.2;
...
scotomarect = CenterRect([0, 0, scotomasize(1), scotomasize(2)], wRect);
    ...
    for trial=1:length(mixtr)
        ...
        oriarray= [theoris(randi(4)) theoris(randi(4)) theoris(randi(4)) theoris(randi(4)) theoris(randi(4)) theoris(randi(4))];
        contrastarray=[ 1 0.9 0.8 0.7 0.6 0.5];
        ...
        while eyechecked<1
            ...
            elseif (eyetime2-trial_time)>=prefixationsquare+ifi*3 && keyCode(RespType(1)) + keyCode(RespType(2)) + keyCode(escapeKey)== 0 %present stimulus
                ...
                %Draw Target
                Screen('DrawTextures', w, TheLetter(trial), [], imageRects, oriarray ,[], contrastarray );
            %    imageRects
