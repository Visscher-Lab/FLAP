...
try
    ...
    if trainingType==2 || trainingType==4
        ...
        JitList = 0:1:90;
        StartJitter=1;
        JitListprog=[0,0,0,4,4,4,8,8,8,10,10,10,14,14,14];
...
%        block_n=length(mixtr_shapes)/3;
 %       block_n=10;
    ...
    if trainingType~=3
        ...
        progtrackend=4; % new variable for prog track setup 7/17 MGR
...
%         end
        ...
        if trainingType==2 || (trainingType==4 && mixtr(trial,3)==2) %if it's a CI trial (training type 2 or 4)
            %             if shapesoftheDay(1) == shapesoftheDay(2) && (mixtr(trial,5)
            %             == 3) % need a condition where trial=1 %note sure if this
            %             conditional is necessary? MGR 7-17
            ...
            %                 Tscat=0;
            %             if shapesoftheDay(1) == shapesoftheDay(2) && (mixtr(trial,5) == 7) && trial == 211 % jitter only carries over for first trial of shape 2 changed 6-13 EC/MGR
            %                 %                     threshShapes(2,2) = thresh(1,2);
            %                 %                     trackthresh(shapesoftheDay(mixtr(trial,1)))= threshShapes(mixtr(trial,1));
            %                 %                     threshperday(expDay,:)=trackthresh;
            %                 Orijit=JitList(thresh(1,2));
            %                 Tscat=0;
            if staircounter(mixtr(trial,1), mixtr(trial,2))>= progtrackend %defined new variable
                Orijit=JitList(thresh(mixtr(trial,1),mixtr(trial,3)));
            else
...
%         end
        ...
        if trainingType==4
            if eyetime2-blocktime>15 %blockdurationtolerance 
