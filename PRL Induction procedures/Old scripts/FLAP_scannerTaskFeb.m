...
% edits Kristina Visscher Feb 2023
...
try
    ...
    stim_duration = round(1/ifi)*ifi %% long stim for testing kmv %%round(.200/ifi) * ifi; % stimulus duration in units of inter-frame intervals
    rest_duration = round(15/ifi)*ifi; % duration of rest block
    ...
    elseif site==1 || site==3
        while ~trigger
            ...
            if keyIsDown==1 && TTL==KbName('t')
                startTime=keyTime; % this variable used to indicate start of MRI
    ...
    for totalblock=1:totalblockfinal  %% pinar -- why name the variable totalblock? This is just the current block, am I missing something?
        BlockStartTime(totalblock) = GetSecs;
        if Isdemo==1
            runnumber=1;
        end
        ...
        if blocktype==4 % rest
            fixationscriptW;
            DrawFormattedText(w, 'x', 'center', xloc, white);%
            RestTime=Screen('Flip',w);
            while GetSecs < RestTime + rest_duration; %  rest for 15 sec %%% must add picking up TRs in here!!

            end
        else
            active=num2str(totalblock);  %% pinar this variable is never used -- should we remove?
            ...
            % note, currently "iti" refers to the time between the end of
            % trial n to the beginning of trial n+1 (not beginning to
            % beginning)
            % LOOP FOR TRIALS %%%%%%%%
...
%                     TimeToStartListening(totalblock,trial)=GetSecs;
%                     TimeToStopListening(totalblock,trial)=TimeToStartListening(totalblock,trial)+((itiprevious-0.5)*TR);
%                 elseif trial>1 && itiforrun(totalblock,trial-1)<=1 %if it's not the first trial of the block and iti is 1TR or less
%                     itiprevious=itiforrun(totalblock,trial-1);
%                     TimeToStartListening(totalblock,trial)=GetSecs;
%                     TimeToStopListening(totalblock,trial)=TimeToStartListening(totalblock,trial)+((itiprevious-0.5)*TR);
...
%                 end
                ...
                if trial==1 % it's the first trial of the block, don't wait to start
                    itiprevious = 0;
                    PreviousCueTime = BlockStartTime(totalblock);
                else %
                    itiprevious = itiforrun(totalblock, trial-1);
                    PreviousCueTime = CueOnsetTime(totalblock,trial-1);
                end
                TimeToStopListening(totalblock,trial)=PreviousCueTime+((itiprevious-0.5)*TR);
                % listen until a while (.5 TRs) before the time we anticipate the
                % next trial. 
                TTLthisTrial(totalblock,trial) = 0; % haven't gotten a TTL yet this trial
                ...
                while GetSecs < TimeToStopListening(totalblock,trial)%resting for previous iti
                    ...
                    if  keyIsDown
                        ...
                        if key==KbName('t');
                            ...
                            elseif k==1 && keypresstime(1,k)>startTime+1  %% Pinar, this is the first trial (did you set k =0 somewhere?
                                TTL_time(j)=keyTime;
                                TTLthisTrial(totalblock,trial)=TTLthisTrial+1; % there was a TTL this trial
                                k=k+1;
                                j=j+1;
                ...
                if TTLthisTrial>0 % there were TTLs recorded this trial
                %if trial>1 && itiprevious>1 && trial<totaltrial % this says, "if we listened for the TTL, above"
                    trialstarttime(totalblock,trial)=TTL_time(j-1)+TR; %   Trials 'start' in sync with the MRI frames, 
                    % start the next trial at a time synced with the TR
                else
                    trialstarttime(totalblock, trial) = PreviousCueTime+((itiprevious)*TR); 
                    % start the next trial at a time that hasn't been synced on this trial
                end
                while GetSecs < trialstarttime(totalblock,trial)-0.001 % %leftover iti wait
                end
