...
dotSizePix = round(dotDiameter * rect(4)); % dot size in pixels
...
% dots in one half ot the symmetrical configuration
numDots = 18;
...
for i = 1:length(dotX)
%    Screen('DrawDots', win, [dotX(i) dotY(i)], dotSizePix, dotColor, [], 1);
    scatter(dotX(i)-Xc, dotY(i)-Yc, dotSizePix,'filled', 'w');
    hold on
end
set(gca,'color',[0.5 0.5 0.5])
xlim([-5 5])
...
for i = 1:length(dotX)
%    Screen('DrawDots', win, [dotX(i) dotY(i)], dotSizePix, dotColor, [], 1);
    scatter(dotX(i)-Xc+theeccentricity_X, dotY(i)-Yc+theeccentricity_Y, dotSizePix,'filled', 'w');
    hold on
end
set(gca,'color',[0.5 0.5 0.5])
xlim([-5 5])
...
figure
%    Screen('DrawDots', win, [dotX(i) dotY(i)], dotSizePix, dotColor, [], 1);
    scatter(dotX-Xc+theeccentricity_X, dotY-Yc+theeccentricity_Y, dotSizePix,'filled', 'w');
