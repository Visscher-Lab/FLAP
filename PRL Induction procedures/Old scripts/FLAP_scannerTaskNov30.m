...
try
    ...
    for totalblock=1:totalactiveblock
        ...
        for trial=1:totaltrial
            ...
            elseif totalblock>1
                stimulusdirection_leftstim=activeblockstim((totalblock*totaltrial)+trial,1);stimulusdirection_rightstim=activeblockstim((totalblock*totaltrial)+trial,2); %what are shown in left and right is set
            ...
            elseif blocktype==3 %6 or 9
                ...
                if stimulusdirection_leftstim_num==1 %9 is shown in the left side
                    Screen('DrawTextures', w, TheGaborsSmall, [], imageRect_offsCIleft'+ [xJitLoc2+xModLoc; yJitLoc2+yModLoc; xJitLoc2+xModLoc; yJitLoc2+yModLoc], theori2,[], Dcontr);
                    imageRect_offsCIleft2(setdiff(1:length(imageRect_offsCIleft),targetcord2),:)=0;
                else %6 is shown in the right side
                    Screen('DrawTextures', w, TheGaborsSmall, [], imageRect_offsCIleft'+ [xJitLoc2+xModLoc; yJitLoc2+yModLoc; xJitLoc2+xModLoc; yJitLoc2+yModLoc], theori2,[], Dcontr);
                    imageRect_offsCIleft2(setdiff(1:length(imageRect_offsCIleft),targetcord2),:)=0;
                end
                Screen('FrameOval', w,gray, imageRect_offsCImaskleft, 22, 22);
                ...
                if site==1
                    while ~timedout
                        ...
                        if keyIsDown
                            RTraw(totalblock,trial)=keyTime;
                            RT(totalblock,trial)=keyTime-ResponseFixationOnsetTime;
                            conditions(1)=logical(cue==1 && stimulusdirection_leftstim_num==1 && responsekey==rightfingerresp);conditions(2)=logical(cue==1 && stimulusdirection_leftstim_num==2 && responsekey==leftfingerresp);conditions(3)=logical(cue==2 && stimulusdirection_rightstim_num==1 && responsekey==rightfingerresp);conditions(4)=logical(cue==2 && stimulusdirection_rightstim_num==2 && responsekey==leftfingerresp); %conditions that are true
                            if any(conditions)==1
                                PsychPortAudio('Start', pahandle1); % feedback for true response
                                ResponseType{totalblock,trial}='correct';
                            else
