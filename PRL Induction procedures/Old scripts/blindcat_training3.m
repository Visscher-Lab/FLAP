...
try
    ...
    BITS=1; % (1=Bits++; 2=Display++; 0=no bits)
    if BITS==1
    ...
    elseif str2num(SUBJECT)>100 && str2num(SUBJECT)<=110
     %   closescript=1;
      %  disp('Error: Subject number > maximum number of subject')
      %  return
        
      
      subcondmixmat=[1 2 1 2 1 2 1 2 1 2];
    ...
    %    break
      ...
      %      typeOfTraining=1
            ...
            typeOfTraining=subcondmixmat(str2num(SUBJECT)-100);  %CAT(1), Classic(2), Noise(3),
        elseif BITS==2
            typeOfTraining=1;
        elseif BITS==0
            typeOfTraining=1;
...
%             
   ...
   % pix_deg_vert = pi * wRect(4) / atan(screencm(2)/v_d/2) / 360 %to calculate the limit of target position
    ...
    %% STAIRCASE
    StartCont=15;  %15
    if expday==1 || expday==numstudydays || expday==numstudydays+1
