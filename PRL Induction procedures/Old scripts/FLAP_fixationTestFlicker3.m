...
try
    prompt={'Participant name', 'session', 'site? UCR(1), UAB(2), Vpixx(3)', 'eye? left(1) or right(2)'};
    
    name= 'Parameters';
    numlines=1;
    defaultanswer={'test','test', '1','2' };
    ...
    smallradius=radiusPRL; % inner circle: scotoma eccentricity
    radiusPRL=radiusPRL+3*pix_deg; %outer circle (3 degrees from the border of the scotoma
    ...
    for trial=1:length(mixtr)
        ...
        while eyechecked<1
            ...
            if (eyetime2-trial_time)>=waittime+ifi*2+precircletime && fixating>400 && stopchecking>1 && counterannulus<AnnulusTime/ifi && counterflicker<FlickerTime/ifi && flickerdone<1 && keyCode(escapeKey) ==0 && (eyetime2-trial_time)<=trialTimeout
            ...
            elseif (eyetime2-trial_time)>=waittime+ifi*2+precircletime && fixating>400 && stopchecking>1 && flickerdone<1  && counterannulus<AnnulusTime/ifi && counterflicker<FlickerTime/ifi && keyCode(escapeKey) ~=0 && (eyetime2-trial_time)<=trialTimeout
                %HERE if I press ESC
                thekeys = find(keyCode);
                closescript=1;
                break;
            elseif (eyetime2-newtrialtime)>=waittime+ifi*2+precircletime && fixating>400 && stopchecking>1 &&  skipcounterannulus>10  && counterflicker<FlickerTime/ifi && (eyetime2-trial_time)<=trialTimeout && keyCode(escapeKey) ==0
               ...
               if exist('flickerstar') == 0
                ...
                annuluspatchFlick2
                if countertarget==round(FlickerTime/ifi)
            ...
            elseif (eyetime2-trial_time)>=waittime+ifi*4+precircletime+FlickerTime && fixating>400 && skipcounterannulus>10 && counterflicker>=FlickerTime/ifi && keyCode(RespType(1)) + keyCode(RespType(2)) + keyCode(RespType(3)) + keyCode(RespType(4)) + keyCode(RespType(5)) + keyCode(escapeKey) ~=0 && stopchecking>1 && (eyetime2-trial_time)<=trialTimeout %present pre-stimulus and stimulus
        ...
        trialflickerduration(trial)=flicker_time_stop-flicker_time_start;
...
%         end
      ...
      %  intervalBetweenFlickerandTrget(trial)=target_time_start-flicker_time_start;
        ...
        if EyeTracker==1
            ...
            EyeSummary.(TrialNum).FlickerFrames=Fixationstatus;
            clear ErrorInfo
