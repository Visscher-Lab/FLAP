...
clc;
commandwindow
addpath([pwd '\utilities2']);%change in mac
%% take information from the user
try
    prompt={'Subject Name', 'Pre or Post Scan','Run Number','site (UAB Disp ++=0 UCR = 1; UAB = 2; UCR Vpixx = 3)'};

    name= 'Subject Name';
    numlines=1;
    defaultanswer={'test','1', '1', '2'};
    ...
    site= str2num(answer{4,:});  %0; 1=bits++; 2=display++
...
indexfingerresp = KbName('LeftArrow');
    ...
    %mixtrVA=[2 1;1 1;2 2;2 2;1 2;2 1;1 2;2 2;1 2;2 2];
   ...
   theori2=180*rand(1,length(imageRect_offs_Small));
    ...
    JitLoc(targetcord)=(pix_deg*offsety(1,:))+yJitLoc(targetcord);
    Tcontr=0.7380; %0.5;
    Dcontr=0.38; %0.5;
    ...
    DrawFormattedText(w, 'index finger',680, 900, white);
    DrawFormattedText(w, 'middle finger',1080, 900, white);
    ...
    for totalblock=1:4
        ...
        Screen('FillPoly',w, tricolor, triangle4coords);
        Screen('FillOval',w,gray,[wRect(3)/2-fix_mask/2, wRect(4)/2-fix_mask/2,wRect(3)/2+fix_mask/2,wRect(4)/2+fix_mask/2]);
        DrawFormattedText(w, 'x', 'center', 'center', white);
