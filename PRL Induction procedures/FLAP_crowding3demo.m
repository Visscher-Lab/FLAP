...
baseName=['./data/' SUBJECT '_FLAPcrowdingacuity' expdayeye num2str(c(1)-2000) '_' num2str(c(2)) '_' num2str(c(3)) '_' num2str(c(4)) '_' num2str(c(5))]; %makes unique filename
...
%KbQueueFlush()
 ...
 while eyechecked<1
  ...
  elseif (eyetime2-pretrial_time)>ifi*65 && fixating>=fixTime/ifi && fixating<1000 && stopchecking>1
    ...
    if whichTask ==3 
         imageRect_offs_cue =[imageRect(1)+(newsamplex-wRect(3)/2)+theeccentricity_X, imageRect(2)+(newsampley-wRect(4)/2)+theeccentricity_Y,...
...
if mixtrAtt(trial,2)==1
                     Screen('DrawTexture',w, theDot, [], imageRect_offs_cue,angloutp,[],1); % lettera a sx del target
