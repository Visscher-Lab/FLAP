...
try
    prompt={'Participant name', 'day','scotoma active', 'demo (0) or session (1)',  'eye? left(1) or right(2)', 'Calibration? no(0), yes (1), validation only (2), MD calibration (3)', 'Eyetracker(1) or mouse(0)?', 'which task? VA (1) or Contrast (2)', 'stimulus size (starting value for VA) in logMAR'};
    
    name= 'Parameters';
    numlines=1;
    defaultanswer={'spot05','1', '0', '1','2','0', '0', '1', '2' };
    ...
    whichTask=str2num(answer{8,:}); %1=VA, 2=contrast
    stimulussizeMAR=str2num(answer{9,:});  %kmv thinks this should be much bigger than 2 degrees.
    stimulussizedeg=(10^stimulussizeMAR)/60;
    ...
    if whichTask==1    % Acuity
        ca=1; %conditions; one
    %    StartSize= 8; %1; %starting size for VA %kmv thinks this line is redundant with the next line, so should be deleted?
                StartSize= stimulussizedeg; %1; %starting size for VA
        Sizelist=[flip(log_unit_up(StartSize, 0.1, 10)) StartSize log_unit_down(StartSize, 0.1, 40)];     
        thresh(1:PRLlocations, 1:ca)=6; % now starting point is 5 log steps above threshold. Threshold would be level 11
