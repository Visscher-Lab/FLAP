...
try
    ...
    name= 'Parameters';
    numlines=1;
    defaultanswer={'test', '1', '3','0', '2', '1', '3' };
    ...
    % first loop: stimulus on screen (flickering) until response, lines 775-938
    % second loop: RSVP starts, then endo or exo cue, lines 950-1226
    HideCursor;
    if Isdemo==1
        ListenChar(2);
    end
    ...
    for trial=1:length(newtrialmatrix)
        ...
        % check if it's time to show instructions
        ...
        stopfixating=0;
        stopfixating2=0;
        T=newtrialmatrix(trial,1); % indicates type of trial. 0= exo cue trial, 1= RSVP trial during endo, 2= RSVP trial during exo, 3= first trial of the series
        ...
        while eyechecked<1
            ...
            if (eyetime2-trial_time)>=0 && (eyetime2-trial_time)<precuetime  && fixating>400  % code review -- is 400 a variable that you think should go in variables in the beginning? MM: no it's a random number, when fixation condition is met, the variable will allow to move to the next loop
                clear stim_star
                clear cueonset
                
            elseif  (eyetime2-trial_time)>precuetime && (eyetime2-trial_time)<precuetime+cuedir+ifi && fixating>500
                
                if  T==0 % it's time for the exo cue
                    offs=1;
            ...
            if newtrialmatrix(trial,1) == 3 % if this is the flip that put the stimulus onscreen, record the time
                ...
                % onscreen. MM: I added the stimulus flip on line 1029
            ...
            if sum(keyCode(RespType(1:6))+keyCode(escapeKey))~= 0 && stopfixating>80   % code review: why 80?  is this a parameter that should be set up top? MM: no it's a random number, when fixation condition is met, the variable will allow to move to the next loop
