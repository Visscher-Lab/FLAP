...
%     end;
    ...
    if trial==1
        ...
        while any(keyCode(Return))== 0
            ...
            Screen('DrawText', w, 'Make your pattern', 0, 0, 255);
            Screen('DrawTextures', w, texture(trial), [], TheTargetloc' , TheTargetori,[], contr );
            Screen('Flip', w)
        end
        targetcord=find(TheTargetloc(:,1));
        distcord=find(TheTargetloc(:,1)==0);
    else     % Presents Cue
        [vbl,cueonset]=Screen('Flip', w); %clear the screen
        if trial==2
            Screen('DrawText', w, 'Now with bakground noise', 0, 0, 255);
        elseif trial==3
            Screen('DrawText', w, 'Now Spatial Noise', 0, 0, 255);
        elseif trial==4
            Screen('DrawText', w, 'Now Orientation Noise', 0, 0, 255);
        end
        
            
            imageRect_offs(targetcord,:)=TheTargetloc(targetcord,:);
